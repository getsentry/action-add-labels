...
def _schedule_next_attempts_bulk(failed_records: list[WebhookPayload]) -> None:
    """Bulk-update multiple failed WebhookPayload records with new attempt counts
    and backoff schedule times in a single UPDATE query per distinct attempt level.

    Groups records by their current attempt count so that each group gets the
    correct backoff interval, then issues one bulk UPDATE per group instead of
    N individual UPDATE queries.
    """
    if not failed_records:
        return

    # Group records by current attempts so each group gets the correct backoff.
    by_attempts: dict[int, list[int]] = {}
    for record in failed_records:
        by_attempts.setdefault(record.attempts, []).append(record.id)
    ...
    for attempts, ids in by_attempts.items():
        ...
        new_schedule_for = now + backoff_delta

        WebhookPayload.objects.filter(id__in=ids).update(
            attempts=new_attempts,
            schedule_for=Greatest(Value(new_schedule_for), "schedule_for"),
        )

    # Update the in-memory objects so callers see the new values.
    for record in failed_records:
        new_attempts = record.attempts + 1
        backoff = BACKOFF_INTERVAL * BACKOFF_RATE**new_attempts
        backoff_delta = datetime.timedelta(minutes=min(backoff, 60))
        record.attempts = new_attempts
        record.schedule_for = max(now + backoff_delta, record.schedule_for)
        ...
        while True:
            ...
            for record in query[:100]:
                ...
                try:
                    ...
                    # Unexpected error (e.g. ValueError from client library).
                    # Reschedule this record individually as a safety net so it
                    # is retried later, then re-raise.
                    failed_records.append(record)
                    _schedule_next_attempts_bulk(failed_records)
                    raise

            # Bulk-update all failed records from this batch in a single query
...
def deliver_message(payload: WebhookPayload) -> None:
    ...
    On success the payload is deleted. On failure a DeliveryFailed exception is
    raised so the caller can collect failed records for bulk retry scheduling.
    """
