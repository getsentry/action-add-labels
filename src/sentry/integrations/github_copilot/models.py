...
class GithubCopilotTaskResponse(BaseModel):
    ...
    task: Optional[GithubCopilotTask] = None

    @root_validator(pre=True)
    def normalize_response(cls, values: dict) -> dict:
        # If the response already has a 'task' key wrapping the task object, use it as-is
        if "task" in values and isinstance(values["task"], dict):
            return values
        # Otherwise, the response contains task fields at the top level (unwrapped format).
        # Wrap them into the expected {"task": {...}} structure.
        if "id" in values and "task" not in values:
            return {"task": values}
        return values
