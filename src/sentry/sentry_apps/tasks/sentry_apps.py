...
def send_webhooks(installation: RpcSentryAppInstallation, event: str, **kwargs: Any) -> None:
    with SentryAppInteractionEvent(
        ...
        if not servicehook:
            ...
            # Check if the installation was deleted, which cascade-deletes the
            # ServiceHook. This is an expected race condition when a Sentry App
            # is uninstalled while webhook tasks are still in-flight.
            fresh_install = app_service.installation_by_id(id=installation.id)
            if not fresh_install:
                lifecycle.record_halt(
                    halt_reason=SentryAppWebhookHaltReason.MISSING_SERVICEHOOK
                )
                return
